...
$(document).ready(function() {
		...
		for (var d in diagram) {
			...
			switch (node.type) {
				...
				case "Visualize":
					...
					break;
				case "Classification":
					html = classification1 + '<option selected="true" disabled="disabled" value="default">'+node.property+'</option>' + classification2 + '<input type="text" name="column" class="form-control column" style="margin:auto auto 15px auto;width:80%;height:70%" onclick="editColumn(this)" value="'+node.column+'"></input>' +  + classification3;
					break;
				case "Regression":
					html = regression1 + '<option selected="true" disabled="disabled" value="default">'+node.property+'</option>' + regression2 + '<input type="text" name="column" class="form-control column" style="margin:auto auto 15px auto;width:80%;height:70%" onclick="editColumn(this)" value="'+node.column+'"></input>' +  + regression3;
					break;
				case "Cleaning":
					html = cleaning1;
					break;
	...
	function generateData() {
		...
		for (m in data.nodes) {
			if (data.nodes[m].property !== "Data Load" && data.nodes[m].property !== "Visualize") {
				let exec = {
					"method": data.nodes[m].type,
					"algorithm":data.nodes[m].property,
					"target": data.nodes[m].column
				}
				if (data.nodes[m].property == "Cleaning") {
					delete exec.target;
					delete exec.algorithm;
				}
				data.execute.push(exec);
			}	
		}
...
function editColumn(element) {
	...
	$('#'+elemid+'  input[name="column"]').keyup(function(){
		for (s in diagram) {
			if (diagram[s]._id == elemid) {
				diagram[s].column = $(this).val();
			}
		}
	});
}
